...
class PagesController(BaseModelController):
    ...
    async def list_pages(self, filters: PageFilterSchema = Query(...), **kwargs):
        ...
        return await self.page_service.get_all_async(filters=filters, user=self.context.request.user)
    ...
    async def detail_page(self, slug: str):
        return await self.page_service.get_one_async(lookup_value=slug, lookup_name='slug')
